...
Note that, in order to build the documentation locally (with ``uv run make html``, as
described below), `Pandoc <https://pandoc.org/installing.html>`_ needs to be
...
2. Compile the docs: ``cd framework/docs``, then ``uv run make html``
...
4. Compile the docs: ``cd framework/docs``, then ``uv run make html``
