...
`guardian-deny` routes `deny:<correct binding>`. `binding-mismatch` routes an
allowed package bound to a deterministic, deliberately different FAB hash.
...
The final JSON is safe to retain: it reports the scenario and expected outcome,
participant IDs, hashes, node IDs, TLS/auth booleans, observed rejection count and
reasons, Guardian/FAB/task-start counts, partition results, and cleanup outcomes, but
no private keys or capability contents. To inspect the generated configuration,
certificates, keys, database, app, and individual process logs, add
`--keep-artifacts`; the output identifies the retained artifact directory. Without
that option, the directory is deleted after the evidence is printed.
...
rejections, and cleanup, while the four process logs record StartRun, per-node routing,
Guardian verification, and the SuperNode fail-closed lifecycle. On a cache hit,
verification still blocks task creation while FAB retrieval remains cached; on a cache
miss, verification blocks both task creation and FAB retrieval.
