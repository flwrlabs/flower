...
@dataclass
class CommandResult:
    """Result of one host command used by the optional local k8s infra
    proof."""
...
def build_tls_material_contract(profile: HarnessProfile) -> dict[str, object]:
    ...
    fingerprint_path = Path(local_path).expanduser() if local_path is not None else None
    ready = fingerprint_path is not None and fingerprint_path.is_file()
    source = "configured-path" if configured_path is not None else "planned-test-ca"
...
def redact_sensitive_data(value: object) -> object:
    """Return ``value`` with credentials, Secret data, and PEM blocks
    redacted."""
...
def redact_command_args(args: Sequence[str]) -> list[str]:
    """Redact credential-bearing command arguments while preserving
    structure."""
    redacted: list[str] = []
    index = 0
...
def build_image_preflight(profile: HarnessProfile) -> dict[str, object]:
    """Return local image checks and the k3d import command for this
    profile."""
    required_images = {
