...
class TestAlembicRun(unittest.TestCase):
    ...
    def test_multiple_connector_accounts_migration_preserves_bindings(self) -> None:
        """Ensure connector rows, run bindings, and active tasks migrate to IDs."""
        engine = self.create_engine()
        try:
            self.upgrade_to_revision(engine, "004925779532")
            with engine.begin() as connection:
                connection.execute(
                    text(
                        ...
                        INSERT INTO connector (
                            connector_id, federation_id, connector_ref,
                            credentials_json, config_json, created_at, deleted_at,
                            created_by, deleted_by
                        ) VALUES (
                            7, '@bob/fed-a', 'slack', '{"token":"secret"}',
                            '{"workspace":"flower"}', '2026-09-24 10:00:00',
                            NULL, 'account-a', NULL
                        )
                        """
                    )
                ...
                connection.execute(
                    text(
                        """
                        INSERT INTO run (
                            run_id, federation_id, primary_task_id,
                            usage_reported_at
                        ) VALUES (101, '@bob/fed-a', 201, '')
                        """
                    )
                )
                connection.execute(
                ...
                connection.execute(
                    text(
                        """
                        INSERT INTO task (
                            task_id, type, run_id, connector_ref, pending_at
                        ) VALUES (
                            201, 'connector', 101, 'slack',
                            '2026-09-24 10:00:00'
                        )
                        """
                    )
            ...
            self.upgrade_to_revision(engine, "8f4785e2dc47")

            with engine.begin() as connection:
                connector = connection.execute(
                    text(
                        """
                        SELECT connector_id, credentials_json, config_json
                        FROM connector WHERE connector_id = 7
                        """
                    )
                ).one()
                self.assertEqual(
                    tuple(connector),
                    (7, '{"token":"secret"}', '{"workspace":"flower"}'),
                )
                self.assertEqual(
                    connection.execute(
                        text(
                            "SELECT connector_id FROM run_connector WHERE run_id = 101"
                        )
                    ).scalar_one(),
                    7,
                )
                self.assertEqual(
                    connection.execute(
                        text("SELECT connector_id FROM task WHERE task_id = 201")
                    ).scalar_one(),
                    7,
                )
                connection.execute(
                    text(
                        """
                        INSERT INTO connector (
                            federation_id, connector_ref, credentials_json,
                            config_json, created_at, deleted_at, created_by,
                            deleted_by
                        ) VALUES (
                            '@bob/fed-a', 'slack', '{}', '{}',
                            '2026-09-24 10:01:00', NULL, 'account-a', NULL
                        )
                        """
                    )
                )
                generated_id = connection.execute(
                    text("SELECT MAX(connector_id) FROM connector")
                ).scalar_one()
                self.assertGreater(generated_id, 7)

            self.downgrade_to_revision(engine, "004925779532")
            with engine.connect() as connection:
                self.assertEqual(
                    connection.execute(
                        text(
                            "SELECT connector_ref FROM run_connector WHERE run_id = 101"
                        )
                    ).scalar_one(),
                    "slack",
                )
                remaining_ids = connection.execute(
                    text(
                        """
                        SELECT connector_id FROM connector
                        WHERE federation_id = '@bob/fed-a'
                          AND connector_ref = 'slack'
                        """
                    )
                ).scalars()
                self.assertEqual(list(remaining_ids), [7])
        finally:
            engine.dispose()
