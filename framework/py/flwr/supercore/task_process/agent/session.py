...
class AgentRuntime:
    ...
    def create_model_response(self, request: JSONObject) -> JSONObject:
        """Create one model response through a child model task."""
        model = request.get("model")
        if not isinstance(model, str) or not model:
            raise ValueError("Model request requires a non-empty string 'model' field.")
        ...
        message = ModelRequest(
            dst_task_id=create_res.task_id,
        ...
        return ModelResponse.from_message(response_message).payload
