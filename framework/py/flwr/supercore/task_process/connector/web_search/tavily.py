...
# ==============================================================================
"""Tavily-backed websearch adapter."""
...
class TavilyWebSearchProvider:
    ...
    def search(self, query: str) -> JSONObject:
        """Execute one Tavily websearch request."""
        if not query.strip():
        ...
        try:
            ...
            raise RuntimeError(
                f"{TAVILY_WEB_SEARCH_PROVIDER} websearch request failed: {exc}"
            ) from exc
        if response.status_code >= 400:
            ...
            raise RuntimeError(
                f"{TAVILY_WEB_SEARCH_PROVIDER} websearch request failed: "
        ...
        try:
            ...
            raise RuntimeError(
                f"{TAVILY_WEB_SEARCH_PROVIDER} websearch returned invalid JSON."
            ) from exc
