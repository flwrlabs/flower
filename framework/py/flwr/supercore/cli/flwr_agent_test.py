...
from .flwr_agent import _parse_args_run_flwr_agent

flwr_agent_module = importlib.import_module("flwr.supercore.cli.flwr_agent")


def test_parse_flwr_agent_requires_token() -> None:
    """The AgentApp process CLI should require a token."""
    with pytest.raises(SystemExit):
        _parse_args_run_flwr_agent().parse_args([])


def test_parse_flwr_agent_rejects_run_once() -> None:
...
def test_parse_flwr_agent_parses_tokenized_invocation() -> None:
...
def test_flwr_agent_parses_args_before_mirroring_output() -> None:
    ...
    with (
        patch.object(flwr_agent_module, "_parse_args_run_flwr_agent", _Parser),
        patch.object(
            flwr_agent_module,
        ...
        flwr_agent_module.flwr_agent()

    mirror_output_to_queue.assert_not_called()


def test_flwr_agent_forwards_cli_args() -> None:
    ...
    with (
        ...
        patch.object(flwr_agent_module, "restore_output", restore_output),
        patch.object(flwr_agent_module, "run_agentapp", run_agentapp),
    ):
        flwr_agent_module.flwr_agent()
