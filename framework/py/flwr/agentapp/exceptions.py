...
class AgentModelError(AgentAppError):
    """Raised when a model task returns a structured error."""

    def __init__(self, error: JsonObject) -> None:
        super().__init__(str(error.get("message", "Model task failed.")))
        self.error = error


class AgentModelTimeoutError(AgentModelError):
    """Raised when a model task does not reply within the timeout."""
