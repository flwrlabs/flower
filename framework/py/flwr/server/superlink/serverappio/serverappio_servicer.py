...
class ServerAppIoServicer(serverappio_pb2_grpc.ServerAppIoServicer):
    ...
    def PullMessages(  # pylint: disable=R0914
        ...
        # Replies carrying an object tree have no inline content in LinkState. Add
        # the small delivery sidecar to the protobuf message so ServerAppIo clients
        # can use the same downstream metric as inline replies without rewriting the
        # object tree.
        for msg_res in messages_res:
            if msg_res.has_error():
                continue
            timings = state.get_delivery_timings(
                msg_res.metadata.reply_to_message_id
            )
            ins_enqueued_at_ms = timings.get("ins_enqueued_at_ms")
            clientapp_delivered_at_ms = timings.get("clientapp_delivered_at_ms")
            if ins_enqueued_at_ms is None or clientapp_delivered_at_ms is None:
                continue
            downstream_ms = max(
                float(clientapp_delivered_at_ms) - float(ins_enqueued_at_ms),
                0.0,
            )
            if not msg_res.has_content():
                msg_res.content = RecordDict()
            metric_record = msg_res.content.metric_records.get(
                "_flwr_network_delivery"
            )
            if metric_record is None:
                metric_record = MetricRecord()
                msg_res.content.metric_records["_flwr_network_delivery"] = metric_record
