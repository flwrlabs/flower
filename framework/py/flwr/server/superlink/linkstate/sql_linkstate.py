...
from .linkstate import LinkState, RunSeries, RunSeriesSummary
from .utils import (
...
class SqlLinkState(LinkState, SqlCoreState):  # pylint: disable=R0904
    ...
    def _run_series_from_row(
        self, row: dict[str, Any], *, run_id: int | None = None
    ) -> RunSeries:
        """Convert a run_series row to a RunSeries domain object."""
        series_id = int64_to_uint64(row["series_id"])
        run_ids = self._run_ids_for_series(series_id)
        context = self._get_run_series_context(series_id, run_id=run_id)
        last_run_status = RunStatus(status="", sub_status="", details="")
        if row["last_run_id"] is not None:
            last_run_id = int64_to_uint64(row["last_run_id"])
            status_rows = self.query(
                ...
                """,
                {"run_id": uint64_to_int64(last_run_id)},
            )
            if status_rows:
                last_run_status = _run_status_from_row(status_rows[0])
    ...
    def _run_series_summary_from_row(self, row: dict[str, Any]) -> RunSeriesSummary:
        """Convert a run_series row to a RunSeriesSummary domain object."""
        series = self._run_series_from_row(row)
    ...
    def _run_ids_for_series(self, series_id: int) -> list[int]:
        """Return run IDs associated with the RunSeries."""
        rows = self.query(
            """
            SELECT run_id
            FROM run
        ...
        return sorted(int64_to_uint64(row["run_id"]) for row in rows)

    def _get_run_series_context(
        self, series_id: int, *, run_id: int | None = None
    ) -> Context | None:
        """Return stored RunSeries context, materialized for a run if requested."""
        rows = self.query(
            """
            SELECT context
            FROM series_context
        ...
        if not rows or rows[0]["context"] is None:
            return None
        context = context_from_bytes(rows[0]["context"])
        if run_id is None:
            return context
        return materialize_run_series_context(
            context,
            run_id=run_id,
            series_id=series_id,
        )
        ...
        *,
        ...
        with self.session():
            ...
            if run_id is not None:
                ...
                run_rows = self.query(
                    """
                    SELECT run_id, federation, series_id
                    FROM run
                    WHERE run_id = :run_id
                    """,
                    {"run_id": sint_run_id},
                ...
                run_row = run_rows[0]
                run_series_id = (
                    int64_to_uint64(run_row["series_id"])
                    if run_row["series_id"] is not None
                    else 0
                )
                if run_row["federation"] != federation:
                    return False
                if run_series_id not in (0, series_id):
                    return False
            ...
            series_rows = self.query(
                """
                SELECT series_id, federation, description
                FROM run_series
                WHERE series_id = :series_id
                """,
            ...
            if not series_rows:
                ...
                if series_row["federation"] != federation:
                    return False
                if description is not None and series_row["description"] != description:
                    self.query(
                        """
                        UPDATE run_series
            ...
            if sint_run_id is not None:
                self.query(
                    ...
                    WHERE run_id = :run_id
                    """,
                    {"series_id": sint_series_id, "run_id": sint_run_id},
                )
                self.query(
                    """
                    UPDATE run_series
            ...
            if context is not None:
                ...
                materialized = materialize_run_series_context(
                    context,
                    run_id=run_id,
                    series_id=series_id,
                )
                self._set_run_series_context(
                    series_id=series_id,
    ...
    def get_run_series_for_run(self, *, run_id: int) -> RunSeries | None:
        ...
        rows = self.query(
            """
            SELECT
                rs.series_id, rs.federation, rs.description, rs.created_at,
                rs.updated_at, rs.last_run_id
            FROM run AS r
            JOIN run_series AS rs ON rs.series_id = r.series_id
            WHERE r.run_id = :run_id
            """,
            {"run_id": uint64_to_int64(run_id)},
        )
        if not rows:
            return None
        return self._run_series_from_row(rows[0], run_id=run_id)
    ...
    def set_run_series_context_for_run(self, *, run_id: int, context: Context) -> None:
        ...
        with self.session():
            ...
            if not run_rows:
                raise ValueError(f"Run {run_id} not found")
            if run_rows[0]["series_id"] is None:
                raise ValueError(f"Run {run_id} is not associated with a RunSeries")

            series_id = int64_to_uint64(run_rows[0]["series_id"])
            series_rows = self.query(
                """
                SELECT series_id
                FROM run_series
            ...
            self._set_run_series_context(series_id=series_id, context=materialized)
            self.query(
                """
                UPDATE run_series
                SET last_run_id = :run_id, updated_at = :updated_at
    ...
    def _set_run_series_context(self, *, series_id: int, context: Context) -> None:
        """Insert or update stored RunSeries context."""
        context_bytes = context_to_bytes(context)
        sint_series_id = uint64_to_int64(series_id)
        rows = self.query(
            """
    ...
    def list_run_series(
        ...
        *,
        ...
        query = """
            ...
            FROM run_series
        """
        if after_series_id is not None:
            query += " WHERE series_id > :after_series_id"
            params["after_series_id"] = uint64_to_int64(after_series_id)
        ...
        rows = self.query(query, params)
        return [self._run_series_summary_from_row(row) for row in rows]
    ...
    def get_run_series(self, *, series_id: int) -> RunSeries | None:
        ...
        if not rows:
            return None
        return self._run_series_from_row(rows[0])
