...
class StateTest(CoreStateTest):
    ...
    def test_ensure_run_series_associates_run_and_context(self) -> None:
        """RunSeries creation should associate a run and store shared context."""
        state: LinkState = self.state_factory()
        run_id = create_dummy_run(state, federation="series-fed")
        context = Context(
            run_id=run_id,
            node_id=SUPERLINK_NODE_ID,
        ...
        stored = state.ensure_run_series(
            series_id=123,
            federation="series-fed",
            run_id=run_id,
            description="test series",
            context=context,
        )
        run = state.get_run_info(run_ids=[run_id])[0]
        series = state.get_run_series(series_id=123)
        run_series = state.get_run_series_for_run(run_id=run_id)
        summaries = state.list_run_series()

        assert stored
        assert run.series_id == 123
        assert series is not None
        assert series.series_id == 123
        assert series.description == "test series"
        assert series.run_ids == [run_id]
        assert series.context is not None
        assert series.context.run_id == run_id
        assert series.context.series_id == 123
        assert series.context.run_config == {"run": "config"}
        assert run_series == series
        assert len(summaries) == 1
        assert summaries[0].series_id == 123
        assert summaries[0].run_ids == [run_id]

    def test_run_series_context_is_shared_across_runs(self) -> None:
        """Associated runs should materialize the same shared series context."""
        state: LinkState = self.state_factory()
        run_id_1 = create_dummy_run(state, federation="series-fed")
        run_id_2 = create_dummy_run(state, federation="series-fed")

        assert state.ensure_run_series(
            series_id=456,
            federation="series-fed",
            run_id=run_id_1,
        )
        ...
        state.set_run_series_context_for_run(
            run_id=run_id_1,
            context=Context(
                run_id=run_id_1,
                node_id=SUPERLINK_NODE_ID,
                node_config={},
                state=RecordDict(),
                run_config={"shared": "first"},
                series_id=456,
            ),
        )
        series_for_run_2 = state.get_run_series_for_run(run_id=run_id_2)

        assert series_for_run_2 is not None
        assert series_for_run_2.context is not None
        assert series_for_run_2.context.run_id == run_id_2
        assert series_for_run_2.context.series_id == 456
        assert series_for_run_2.context.run_config == {"shared": "first"}
        ...
        state.set_run_series_context_for_run(
            ...
            context=Context(
                ...
                node_config={},
                state=RecordDict(),
                run_config={"shared": "second"},
                series_id=0,
            ),
        )
        series_for_run_1 = state.get_run_series_for_run(run_id=run_id_1)

        assert series_for_run_1 is not None
        assert series_for_run_1.context is not None
        assert series_for_run_1.context.run_id == run_id_1
        assert series_for_run_1.context.series_id == 456
        assert series_for_run_1.context.run_config == {"shared": "second"}

    def test_list_run_series_paginates_by_series_id(self) -> None:
        """RunSeries list should support cursor-style pagination."""
        state: LinkState = self.state_factory()
        assert state.ensure_run_series(series_id=10, federation="series-fed")
        assert state.ensure_run_series(series_id=20, federation="series-fed")
        assert state.ensure_run_series(series_id=30, federation="series-fed")

        summaries = state.list_run_series(after_series_id=10, limit=1)

        assert [summary.series_id for summary in summaries] == [20]
        assert state.list_run_series(limit=0) == []
        with self.assertRaises(AssertionError):
            state.list_run_series(limit=-1)

    def test_ensure_run_series_rejects_invalid_association(self) -> None:
        """RunSeries association should reject invalid IDs and mismatched runs."""
        state: LinkState = self.state_factory()
        run_id = create_dummy_run(state, federation="series-fed")
        other_run_id = create_dummy_run(state, federation="series-fed")

        assert not state.ensure_run_series(
            series_id=0,
            federation="series-fed",
            run_id=run_id,
        )
    ...
    def test_delete_run_series_keeps_associated_runs(self) -> None:
        """Deleting RunSeries should not delete or mutate associated runs."""
        state: LinkState = self.state_factory()
        run_id = create_dummy_run(state, federation="series-fed")
        assert state.ensure_run_series(
            series_id=123,
        ...
        assert state.delete_run_series(series_id=123)
        assert not state.delete_run_series(series_id=123)
        assert state.get_run_series(series_id=123) is None
        assert state.get_run_series_for_run(run_id=run_id) is None
        run = state.get_run_info(run_ids=[run_id])[0]
        assert run.run_id == run_id
        assert run.series_id == 123
