...
the repository root. Use Python `3.12.13` for local agent commands; CI also checks
...
```bash
cd framework
uv run --no-sync --python=3.12.13 <command>
```
...
```bash
cd framework
uv sync --locked --python=3.12.13 --all-extras --all-groups
```
...
uv run --no-sync --python=3.12.13 ./dev/test.sh false

# Full framework quality gate used by pre-commit/CI
uv run --no-sync --python=3.12.13 ./dev/test.sh

# Narrow tests
uv run --no-sync --python=3.12.13 python -m pytest py/flwr/path/to_test.py
uv run --no-sync --python=3.12.13 python -m pytest py/flwr -k "name"

# Targeted type/lint checks
uv run --no-sync --python=3.12.13 python -m mypy py
uv run --no-sync --python=3.12.13 python -m ruff check py/flwr --no-respect-gitignore
uv run --no-sync --python=3.12.13 python -m pylint --ignore=py/flwr/proto py/flwr
```
...
```bash
cd framework
uv run --no-sync --python=3.12.13 python -m dev.generate_migration "Describe schema change"
```
...
```bash
uv run --no-sync --python=3.12.13 ./dev/check-migrations.sh
```
...
```bash
uv run --no-sync --python=3.12.13 ./dev/build-docs.sh
```
...
uv run --no-sync --python=3.12.13 ./dev/test-wheel.sh
```
