...
1. select only the provider or providers needed for this task; and
1. submit the prompt.
...
| --------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| Slack     | Search messages; list conversations; read conversation history; read thread replies | Content visible to the connected Slack user; read-only        |
| Notion    | Search shared pages and data sources; read page blocks                              | Pages and data sources shared during authorization; read-only |
| GitHub    | Search code in one public repository; read one public UTF-8 file                    | Public repositories only; read-only                           |
| Attio     | Search records; list meetings and call recordings; read call transcripts            | Connected Attio workspace; read-only                          |
