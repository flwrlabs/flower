...
```bash
cd framework
uv run --no-sync --python=3.12.13 python -m dev.generate_migration "Description"
```
...
```bash
cd framework
uv run --no-sync --python=3.12.13 python -m dev.generate_migration "Widen integer columns"
```

```bash
cd framework
uv run --no-sync --python=3.12.13 python -m dev.generate_migration --head flwr@head "Widen integer columns"
```
