...
```bash
uv run --no-sync --python=3.10.19 python -m dev.generate_migration "Description"  # Create migration
```
...
`python -m dev.generate_migration` instead, then review the generated revision and make
only the minimal adjustments needed. This helps avoid schema drift between SQLAlchemy
models and committed migrations.
